import warnings
warnings.filterwarnings("ignore", message="The numpy.array_api submodule is still experimental. See NEP 47.")
...
def start_presentation():
    global root, folderPath, from_upload_page
    ...
    closeCounter = 0
    closeThreshold = 3  # 
    closeGesture = [1, 1, 1, 1, 1]  # Gesture to close the application

    # Hand Detector
    detector = HandDetector(detectionCon=0.8,maxHands=1)

    key = None  # Ensure key is defined before the finally block
    ...
    try:
        ...
        if from_upload_page and folderPath == "presentation" and 'imgNumber' in locals():
...
if __name__ == "__main__":
    global root, selected_pptx_file, loading_screen, folderPath, folder_name, from_upload_page
    selected_pptx_file = None
    from_upload_page = False

    # Print command-line arguments for debugging
    print("Command-line arguments:", sys.argv)

    if len(sys.argv) > 1 and (sys.argv[1] == "upload" or sys.argv[-1] == "upload"):
        folderPath = "presentation"
        from_upload_page = True
        root = Tk()
